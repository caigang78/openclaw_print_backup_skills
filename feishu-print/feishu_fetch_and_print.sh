#!/bin/bash
# feishu_fetch_and_print.sh — Download files from a Feishu group chat and send to printer.
# Downloaded files are saved to <backup_root>/YYYY-MM-DD/<type>/ before printing.
#
# Basic usage (print latest file):
#   PRINTER=MyPrinter ./feishu_fetch_and_print.sh
#
# Smart-matching usage (agent sets env vars based on user intent):
#   PRINTER=MyPrinter LIMIT=2 ./feishu_fetch_and_print.sh                  # latest 2 files
#   PRINTER=MyPrinter NAME_PREFIX=report ./feishu_fetch_and_print.sh       # files starting with "report"
#   PRINTER=MyPrinter MINUTES=5 FILE_TYPE=pdf ./feishu_fetch_and_print.sh  # PDFs from last 5 minutes
#   PRINTER=MyPrinter LIMIT=3 MINUTES=10 ./feishu_fetch_and_print.sh       # up to 3 files from last 10 minutes
...
if [ -z "$PRINTER" ]; then
    echo "Error: set the PRINTER environment variable to specify a printer, e.g.: PRINTER=MyPrinter ./feishu_fetch_and_print.sh"
